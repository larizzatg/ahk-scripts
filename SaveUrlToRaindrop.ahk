...
SaveUrlToRaindrop()
{
    ...
    HttpObj.setRequestHeader("Authorization", "Bearer {RAINDROP_TEST_TOKEN}") ; Replace {RAINDROP_TEST_TOKEN} with the test token of raindrop
    Data := ({"order": 0, "link": CurrentUrl})
    ...
    If (Status != "200")
    {
        MsgBox,  0, "There was an error", "%result%"
    }

    MsgBox, , "Added Raindrop", "%CurrentUrl%", 1
    Clipboard := BackupClipboard
}
